...
import { findLink } from '../lib/links.js';
...
async function handleLinks(message) {
  ...
  if (cfg.ignoreStaff !== false && message.member?.permissions.has(PermissionFlagsBits.ManageMessages)) return false;
  if ((cfg.exemptChannelIds ?? []).includes(message.channel.id)) return false;

  const link = findLink(message.content);
  ...
  if (cfg.warnUser) {
    const notice = await message.channel
      .send({ ...simple({ accent: config.colors.warning, lines: [`<@${message.author.id}>, links are not allowed here and your message was removed.`] }), allowedMentions: { users: [message.author.id] } })
      .catch(() => null);
  ...
  await logAction(message.guild, {
    ...
    extra: [`**Channel:** <#${message.channel.id}>`, `**Link:** \`${link.slice(0, 200)}\``],
...
export async function execute(message) {
  ...
  if (await handleLinks(message)) return;

  if (config.blacklist.ignoreStaff !== false && message.member?.permissions.has(PermissionFlagsBits.ManageMessages)) return;

  const words = getWords(message.guild.id);
  if (words.length === 0) return;

  const matched = findMatch(message.content, words, config.blacklist.matchMode);
  if (!matched) return;
