...
import { claimSingleInstance } from './lib/singleInstance.js';

// Two processes on one token both answer every interaction and both rewrite the
// data files, which is what made tickets roll back. Refuse to be the second.
claimSingleInstance(env.token, `Hanaki Manager (guild ${env.guildId ?? '?'})`);
